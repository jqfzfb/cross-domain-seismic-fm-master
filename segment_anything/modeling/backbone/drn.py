...
class DRN(nn.Module):
    ...
    def __init__(self, block, layers, arch='D',
        ...
        if arch == 'C':
            self.conv1 = nn.Conv2d(img_ch, channels[0], kernel_size=7, stride=1,
            ...
            self.layer0 = nn.Sequential(
                nn.Conv2d(img_ch, channels[0], kernel_size=7, stride=1, padding=3,
                          bias=False),
...
def drn_d_54(BatchNorm, img_ch=3, pretrained=False):
    model = DRN(Bottleneck, [1, 1, 3, 4, 6, 3, 1, 1], arch='D', BatchNorm=BatchNorm, img_ch=img_ch)
    if pretrained:
...
def drn_d_105(BatchNorm, img_ch, pretrained=False):
