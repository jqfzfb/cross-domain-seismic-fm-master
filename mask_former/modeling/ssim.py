...
# zfbi 23/02/24
def _fspecial_gaussian1d(size, channel, sigma):
    coords = torch.tensor([(x - (size - 1.) / 2.) for x in range(size)])
    coords = -coords ** 2 / (2. * sigma ** 2)
    grid = coords.view(1, -1)
    grid = grid.softmax(-1)
    kernel = grid.view(1, 1, size)
    kernel = kernel.expand(channel, 1, size).contiguous()
    return kernel
...
# zfbi 23/02/24
def _ssim1d(output, target, max_val, k1, k2, channel, kernel):
    c1 = (k1 * max_val) ** 2
    c2 = (k2 * max_val) ** 2

    mu1 = F.conv1d(output, kernel, groups=channel)
    mu2 = F.conv1d(target, kernel, groups=channel)

    mu1_sq = mu1 ** 2
    mu2_sq = mu2 ** 2
    mu1_mu2 = mu1 * mu2

    sigma1_sq = F.conv1d(output * output, kernel, groups=channel) - mu1_sq
    sigma2_sq = F.conv1d(target * target, kernel, groups=channel) - mu2_sq
    sigma12 = F.conv1d(output * target, kernel, groups=channel) - mu1_mu2
...
# zfbi 23/02/24
def ssim_loss1d(input, target, max_val, filter_size=7, k1=0.01, k2=0.03,
    ...
    dim = input.dim()
    if   dim == 2:
        input = input.expand(1, input.dim(-2), input.dim(-1))
        target = target.expand(1, target.dim(-2), target.dim(-1))
    elif dim != 3:
        raise ValueError('Expected 2 or 3 dimensions (got {})'.format(dim))

    _, channel, _ = input.size()

    if kernel is None:
        kernel = _fspecial_gaussian1d(filter_size, channel, sigma)
    kernel = kernel.to(device=input.device)

    ret, _ = _ssim1d(input, target, max_val, k1, k2, channel, kernel)
...
class SSIMLoss1d(_Loss):
    ...
    def __init__(self, channel=3, filter_size=7, k1=0.01, k2=0.03, sigma=1.5, size_average=None, reduce=None, reduction='mean'):
        super(SSIMLoss1d, self).__init__(size_average, reduce, reduction)
        self.filter_size = filter_size
        self.k1 = k1
        self.k2 = k2
        self.sigma = sigma
        self.kernel = _fspecial_gaussian1d(filter_size, channel, sigma)

    def forward(self, input, target, max_val=1.):
        return ssim_loss1d(input, target, max_val=max_val, filter_size=self.filter_size, k1=self.k1, k2=self.k2,
